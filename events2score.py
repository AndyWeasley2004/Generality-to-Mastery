...
def token_to_event(tokens):
    ...
    for token in tokens:
        ...
        if len(parts) == 3 and parts[0] in ["Note", "Time", "Chord"]:
            ...
            value = parts[2]                  # e.g. "60" / "4/4"
            if parts[0] == 'Note': value = int(value) # convert duration and pitch to int
        elif len(parts) == 2:
            # like composers, beat
            name, value = parts
            if name in ['Beat', 'Tempo']: value = int(value)
        else:
...
# used for reconstruction, you can change the interface
# to fit different folder structure
if __name__ == '__main__':
    dataset = 'schumann_score'
    data_root = f'../score_data/full/{dataset}/events'
    ...
    for file in tqdm(files):
        ...
        output_path = os.path.join(output_root, file[:-4]+'.mid')
        data = pickle_load(filepath)[0]
        convert_sequence_to_midi(data, output_midi_path=output_path)
